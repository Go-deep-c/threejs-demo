...
// Floor
const floor = new THREE.Mesh(new THREE.PlaneGeometry(8,8), new THREE.MeshStandardMaterial({color:0xcccccc}));
...
window.addEventListener('pointermove',e=>{if(!isDragging)return; const dx=e.clientX-prevX,dy=e.clientY-prevY; yaw+=dx*0.01; pitch+=dy*0.01; prevX=e.clientX; prevY=e.clientY;});
...
function animate(){ requestAnimationFrame(animate); camera.position.x=Math.sin(yaw)*6; camera.position.z=Math.cos(yaw)*6; camera.lookAt(0,1,0); renderer.render(scene,camera); }
animate();